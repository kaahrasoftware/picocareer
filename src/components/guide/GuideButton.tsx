...
export function GuideButton({
  ...
  if (floating) {
    return <button className={`guide-floating-button ${!hasSeen ? 'guide-button-new' : ''}`} onClick={handleStartGuide} title="View page guide">
        
        <span className="sr-only">Help & Guide</span>
      </button>;
  }
  return <Button variant="ghost" size="sm" className={`relative ${className} ${!hasSeen ? 'guide-button-new' : ''}`} onClick={handleStartGuide} title="View page guide">
