  ...
  if (compact) {
    return (
      <>
        <Card 
          className={cn(
            "h-full transition-all duration-300 overflow-hidden border cursor-pointer group", 
            cardGradient,
            "transform hover:translate-y-[-4px] hover:shadow-lg",
        ...
        >
                  ...
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        isBookmarked ? "text-primary hover:text-primary/80" : "text-muted-foreground hover:text-foreground",
          ...
          <CardFooter className="pt-2">
            <Button 
              variant="ghost" 
              className="p-0 h-auto text-primary hover:text-primary/80 hover:bg-transparent group-hover:underline"
              onClick={openDetailsDialog}
            >
  ...
  return (
    <>
      ...
      >
        ...
        <CardContent className="pb-3">
          ...
          <div className="flex flex-wrap gap-1 mt-3">
            {scholarship.category?.slice(0, 2).map((cat) => (
              <Badge key={cat} variant="outline" className="bg-background/80 text-primary border-primary/30 hover:bg-primary/10">
                {cat}
              </Badge>
            ))}
          ...
          {scholarship.eligibility_criteria && (
            <div className="mt-4 text-xs text-muted-foreground">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="font-medium">Eligibility highlights:</span>
              </div>
              <ul className="list-disc list-inside space-y-0.5 pl-1">
                {scholarship.eligibility_criteria.gpa_requirement && (
                  <li>GPA: {scholarship.eligibility_criteria.gpa_requirement}+</li>
                )}
                {scholarship.eligibility_criteria.academic_year?.length > 0 && (
                  <li>Year: {scholarship.eligibility_criteria.academic_year.slice(0, 2).join(", ")}{scholarship.eligibility_criteria.academic_year.length > 2 ? "..." : ""}</li>
                )}
                {scholarship.eligibility_criteria.major?.length > 0 && (
                  <li>Major: {scholarship.eligibility_criteria.major.slice(0, 1).join(", ")}{scholarship.eligibility_criteria.major.length > 1 ? "..." : ""}</li>
                )}
              </ul>
            </div>
          )}
        ...
        {/* Featured indicator */}
        {scholarship.featured && (
          <div className="absolute top-0 right-0 w-0 h-0 border-t-[60px] border-r-[60px] border-t-amber-500/80 border-r-transparent transform rotate-0 -translate-y-0 translate-x-0" />
        )}
      </Card>
