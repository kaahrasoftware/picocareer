...
import { CalendarClock, MapPin, Globe, BookmarkPlus, Bookmark, Share2, ArrowLeft, Clock, Building, ExternalLink, Users, Eye, DollarSign } from "lucide-react";
...
export default function OpportunityDetails() {
  ...
  return <div className="container px-4 py-8 mx-auto">
      ...
      duration: 0.3
        ...
        <Button variant="ghost" className="mb-6 flex items-center gap-1 group" onClick={() => navigate("/opportunities")}>
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2">
                <Badge className={cn("capitalize font-medium", typeStyles.bg, typeStyles.text, typeStyles.border, typeStyles.hoverBg, "border")}>
                  {opportunity.opportunity_type}
                </Badge>
                
                {opportunity.featured && <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-200">
                    Featured
                  </Badge>}
                
                {opportunity.remote && <Badge variant="outline" className="flex items-center gap-1">
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                {opportunity.deadline && <div className="flex items-center gap-1.5 text-sm">
                    <CalendarClock className="h-4 w-4 text-muted-foreground" />
                    <span>Deadline: <span className="font-medium">{formatDate(opportunity.deadline)}</span></span>
                  </div>}
                
                {opportunity.location && <div className="flex items-center gap-1.5 text-sm">
                    <MapPin className="h-4 w-4 text-muted-foreground" />
                    <span>{opportunity.location}</span>
                  </div>}
                
                {opportunity.compensation && <div className="flex items-center gap-1.5 text-sm">
            ...
            {opportunity.categories && opportunity.categories.length > 0 && <div className="pt-2">
                <h3 className="text-sm font-medium mb-2">Categories</h3>
                <div className="flex flex-wrap gap-2">
                  {opportunity.categories.map(category => <Badge key={category} variant="secondary">{category}</Badge>)}
                </div>
              </div>}

            {opportunity.tags && opportunity.tags.length > 0 && <div className="pt-2">
          ...
          <div className="md:col-span-1">
            <div className="bg-card border rounded-lg shadow-sm p-6 sticky top-24 space-y-6">
              ...
              <div className="flex flex-col gap-3">
                {opportunity.application_url ? <Button asChild className="w-full gap-2">
                    <a href={opportunity.application_url} target="_blank" rel="noopener noreferrer">
                      <ExternalLink className="h-4 w-4" />
                      Check it out
                    </a>
                  </Button> : <Button className="w-full" onClick={handleApply}>
                    Apply Now
                  </Button>}

                <Button variant="outline" className="w-full flex items-center justify-center gap-2" onClick={() => toggleBookmark()} disabled={bookmarkLoading}>
                  {isBookmarked ? <>
                ...
                <Button variant="ghost" className="w-full flex items-center justify-center gap-2" onClick={handleShare}>
                  <Share2 className="h-4 w-4" />
                  Share
                </Button>
              </div>

              {opportunity.profiles && <div className="pt-4 border-t border-border">
                  <h3 className="font-medium mb-2 text-sm">Posted by</h3>
                  <div className="flex items-center gap-3">
                    {opportunity.profiles.avatar_url ? <Avatar>
                        <AvatarImage src={opportunity.profiles.avatar_url} alt={opportunity.profiles.full_name || "User"} />
      ...
      <Dialog open={applyDialogOpen} onOpenChange={setApplyDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <ApplicationForm opportunityId={opportunity.id} opportunityTitle={opportunity.title} onComplete={() => setApplyDialogOpen(false)} />
        </DialogContent>
      </Dialog>
    </div>;
}