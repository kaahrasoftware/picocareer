...
export function generateHeader(
  ...
  switch (headerStyle) {
    case 'banner':
      return `
        <div style="background: linear-gradient(135deg, ${styles.primary}, ${styles.secondary}); 
                    color: white; padding: 32px; text-align: center; margin-bottom: 24px;">
          ${logoUrl ? `<img src="${logoUrl}" alt="Logo" style="height: 40px; margin-bottom: 16px;">` : ''}
          <h1 style="margin: 0; font-size: 28px; font-weight: 600;">${displayTitle}</h1>
        </div>
      `;
    ...
    case 'minimal':
      return `
        <div style="padding: 24px; margin-bottom: 24px;">
          ...
          <h1 style="margin: 0; font-size: 24px; font-weight: 600; color: ${styles.primary};">${displayTitle}</h1>
    ...
    default:
      return `
        <div style="text-align: center; padding: 24px; margin-bottom: 24px;">
          ${logoUrl ? `<img src="${logoUrl}" alt="Logo" style="height: 40px; margin-bottom: 16px;">` : ''}
          <h1 style="margin: 0; font-size: 26px; font-weight: 600; color: ${styles.primary};">${displayTitle}</h1>
        </div>
      `;
  }
}
