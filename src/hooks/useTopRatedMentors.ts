    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (!data || data.length === 0) {
          console.log('No top rated mentors found');
          return [];
        }
        ...
        const formattedMentors = data.map(mentor => ({
          ...
          academic_major: mentor.major?.name || null,
          career_title: mentor.career?.title || null
        })) as ProfileWithDetails[];

        console.log('Fetched mentors successfully:', formattedMentors.length);
        return formattedMentors;
      } catch (err) {
        console.error('Error in useTopRatedMentors hook:', err);
        return [];
      }
    },
    staleTime: 1000 * 60 * 5,
