...
-- Add conditional logic fields to assessment_questions table
ALTER TABLE assessment_questions 
ADD COLUMN profile_type text[] DEFAULT NULL,
ADD COLUMN target_audience text[] DEFAULT NULL,
ADD COLUMN prerequisites jsonb DEFAULT NULL,
...
-- Add profile detection to career_assessments table
ALTER TABLE career_assessments
ADD COLUMN detected_profile_type profile_type_enum DEFAULT NULL,
ADD COLUMN profile_detection_completed boolean DEFAULT false;

-- Insert profile detection questions
...
-- Update existing questions to have profile types (make them universal for now)
UPDATE assessment_questions 
SET profile_type = '["middle_school", "high_school", "college", "career_professional"]'::text[],
    target_audience = '["all"]'::text[]
WHERE profile_type IS NULL;

-- Add index for better performance on conditional queries
CREATE INDEX idx_assessment_questions_profile_type ON assessment_questions USING GIN (profile_type);
CREATE INDEX idx_assessment_questions_target_audience ON assessment_questions USING GIN (target_audience);
